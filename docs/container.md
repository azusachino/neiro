...
- A failed pull or push raises `HistoryError`. The local commit stays; call `await vault.sync()` to retry. A pull whose rebase conflicts is aborted, leaving the clone as it was, and its `HistoryError` means a person must reconcile the two histories: retrying will not.
